    ...
    <header className="sticky top-0 z-40 w-full border-b bg-background/80 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
        <Link to="/" className="flex items-center gap-2">
          <img src={logoUrl} alt="ClauGas Express — Hub Buea" className="h-10 w-10 rounded-full object-cover ring-1 ring-border" />
        ...
        <div className="flex items-center gap-2">
          <LanguageSwitcher />
          {signedIn ? (
            <Button size="sm" onClick={() => navigate({ to: "/dashboard" })}>{t("nav.dashboard")}</Button>
          ) : (
            <>
              <Button size="sm" variant="ghost" className="hidden sm:inline-flex" onClick={() => navigate({ to: "/auth" })}>
                {t("nav.login")}
              </Button>
              <Button size="sm" onClick={() => navigate({ to: "/auth" })}>{t("nav.register")}</Button>
            </>
          )}
          <Button size="icon" variant="ghost" className="md:hidden" onClick={() => setOpen((o) => !o)} aria-label={t("nav.menu")}>
